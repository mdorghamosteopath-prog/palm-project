...
import{Users,Heart,Camera,ChevronRight,Check,X,LockKeyhole,Sparkles}from'lucide-react';
...
export default function SocialHub({history,onScan,onOpen,onRename,onDelete,unlocked=false,onUnlock}){
 ...
 const group=useMemo(()=>{
  ...
  const pairs=[];
  for(let i=0;i<people.length;i++)for(let j=i+1;j<people.length;j++)pairs.push({a:people[i],b:people[j],c:buildCompatibility(people[i],people[j])});
  pairs.sort((a,b)=>b.c.score-a.c.score);
  return{
   pairs,
   avg:Math.round(pairs.reduce((s,p)=>s+p.c.score,0)/pairs.length),
   leader:[...people].sort((a,b)=>b.scores[1]-a.scores[1])[0],
   connector:[...people].sort((a,b)=>b.scores[0]-a.scores[0])[0],
   wild:[...people].sort((a,b)=>b.scores[2]-a.scores[2])[0]
  };
 },[people]);
 ...
 const randomCard=list=>setCard(list[Math.floor(Math.random()*list.length)]);
 const game=(title,sub,list)=><section><button className="backLink" onClick={()=>{setMode('hub');setCard(null)}}>← Retour</button><div className="eyebrow"><Sparkles size={14}/> PALM PLAY</div><h2>{title}</h2><p>{sub}</p>{card?<div className="matchHero"><span>{card[0]}</span><h2>{card[1]}</h2></div>:<div className="resultCard"><div className="rare">PRÊTS ?</div><h2 className="profile">Une carte. Une réaction.</h2><p>Le but est de lancer les discussions et découvrir ce que les autres pensent vraiment.</p></div>}<button className="primary" onClick={()=>randomCard(list)}>{card?'CARTE SUIVANTE':'LANCER LE JEU'} <ChevronRight/></button></section>;
 ...
 if(mode==='duoGame')return unlocked?game('Duo Challenge','Vérités, prédictions et mini-défis à faire à deux.',DUO):<section><button className="backLink" onClick={()=>setMode('hub')}>← Retour</button>{paywall('Duo Challenge','Des cartes Vérité, Prédiction et Défi pour prolonger chaque Match.')}</section>;
 if(mode==='groupGame')return unlocked?game('Group Game','Passe le téléphone. Votez, débattez, révélez-vous.',GROUP):<section><button className="backLink" onClick={()=>setMode('hub')}>← Retour</button>{paywall('Group Game','Un jeu de groupe intégré à PALM : votes, vérités et actions pour toute la bande.')}</section>;
